...
import type { UseApiOptions, ApiState, ApiReturn } from "./useApi.types";

function useApi<TResponse = unknown, TBody = unknown>(
  url: string,
  options: UseApiOptions<TBody> = {}
): ApiReturn<TResponse, TBody> {
  const {
    method = "GET",
    body,
    immediate = false,
  ...
  // Store options in a ref to avoid recreating execute on every render
  ...
  const execute = useCallback(
    async (overrideBody?: TBody): Promise<TResponse | undefined> => {
      try {
